...
# Create defaultdict for storing positions of words in group A
group_a = defaultdict(list)

# Read group A words and store their 1-based positions
for i in range(1, n + 1):
    word = input()
    group_a[word].append(i)
# group_a looks like ("a",[1,2,4])
# Read group B words and print their positions from group A
for _ in range(m):
    query_word = input()
    if query_word in group_a:
        print(' '.join(map(str, group_a[query_word])))
    else:
        print(-1)
